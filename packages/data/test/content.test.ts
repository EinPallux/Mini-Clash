...
const REQUIRED_ANIM_STATES = ['idle', 'run', 'attack', 'cast_q', 'cast_w', 'cast_r', 'death', 'spawn', 'dance'];

describe('content schema validation', () => {
  it('champions parse', () => {
    for (const c of CHAMPION_LIST) {
      const res = championSchema.safeParse(c);
      expect(res.success, `${c.id}: ${JSON.stringify(res.success ? '' : res.error.issues[0])}`).toBe(true);
    }
  });
  ...
  it('units, projectiles, buffs, map, fx, cues parse', () => {
    ...
    for (const p of Object.values(PROJECTILES)) expect(projectileSchema.safeParse(p).success, p.id).toBe(true);
    ...
    for (const f of Object.values(FX)) expect(fxTimelineSchema.safeParse(f).success, f.id).toBe(true);
    for (const s of Object.values(SOUND_CUES)) expect(synthCueSchema.safeParse(s).success, s.id).toBe(true);
  });
});
  ...
  it('ability actions reference existing projectiles, buffs, units', () => {
    for (const c of CHAMPION_LIST) {
      for (const ability of Object.values(c.abilities)) {
        ...
        walkActions(all, (a) => {
          if (a.t === 'projectile') expect(PROJECTILES[a.proj], `${c.id}: proj ${a.proj}`).toBeDefined();
          ...
          if (a.t === 'wall' && a.allyBuff) expect(BUFFS[a.allyBuff], `${c.id}: buff ${a.allyBuff}`).toBeDefined();
        });
      }
    }
  });
  ...
  it('fx sound ops reference existing cues; champions have core fx timelines', () => {
    for (const f of Object.values(FX)) {
      for (const ev of f.events) {
        if (ev.op.t === 'sound') expect(SOUND_CUES[ev.op.cue], `${f.id}: cue ${ev.op.cue}`).toBeDefined();
      }
    }
    for (const c of CHAMPION_LIST) {
