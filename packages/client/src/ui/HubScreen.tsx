...
import { ChampionsScreen } from './hub/ChampionsScreen';
import { HistoryScreen } from './hub/HistoryScreen';
import { HubShell, type HubTab, OfflineRewardNote, useScrollReset } from './hub/HubShell';
import { ProfileScreen } from './hub/ProfileScreen';
import { QuestsScreen } from './hub/QuestsScreen';
import { ShopScreen } from './hub/ShopScreen';
...
export function HubScreen(): React.ReactElement {
  ...
  useScrollReset(tab);
  // Ownership can change on any tab (a purchase, a match reward), so the hub
  // re-reads it whenever the player comes back to Play.
  const refresh = useAccount((s) => s.refresh);
  useEffect(() => {
    if (tab === 'play' && useAccount.getState().status === 'ready') void refresh();
  }, [tab, refresh]);

  return (
    <HubShell tab={tab} onTab={setTab} onSettings={() => setSettingsOpen(true)}>
      {tab === 'play' && <PlayTab />}
      {tab === 'champions' && <ChampionsScreen />}
      {tab === 'shop' && <ShopScreen />}
...
function PlayTab(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent): void => {
      ...
      if (e.code === 'Enter' && !friendsOpen && !document.querySelector('.modal-veil')) {
        uiSound('ui_click');
        setMatchMode('bridge');
        goto('select');
      }
    ...
    <>
      ...
      <div className="mode-row">
        <button type="button" className="mode-card" onClick={() => play('bridge')}>
          ...
          <div className="hero" style={{ background: 'linear-gradient(135deg, #b23a3a, #6e1c1c)' }}>
            <span
              className="glyph"
              style={{
        ...
        <button type="button" className="mode-card" onClick={() => play('training')}>
          ...
          <div className="hero" style={{ background: 'linear-gradient(135deg, #2ea860, #1c6e3e)' }}>
            <span
              className="glyph"
              style={{
        ...
        >
          ...
          <div className="hero" style={{ background: 'linear-gradient(135deg, #3b6fd4, #1d3a78)' }}>
            <span
              className="glyph"
              style={{
      ...
      <div className="hintbar on-light">Mini Clash v0.7 — The Hub</div>
