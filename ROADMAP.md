...
- [x] Harness: no generic augment > 65% pick-when-offered or > 56% win-delta; report auto-generated per run. *(`botmatch.mjs` prints the report every run — per card: offers, pick-when-offered, winrate, n — with signatures broken out separately, because a card written for exactly one kit being a near-auto-pick on that kit is the design, not a flag. **Pick rail: passes**, top generic at **39%** against a 65% cap.
  **The interesting part is how it got there.** The first 60-match run flagged Undying Contract at **74%**, so it was nerfed (30% revive instead of 40%, and the arriving half locked out for a swap cooldown). Re-running the same seed, the pick rate went **up, to 79%** — because `scoreOffer` never reads an augment's parameters at all. It scored on rarity, category and tags, so nothing about the nerf was visible to the thing doing the measuring. Every top-picked generic across both runs was prismatic; the rail was reporting on a **1 / 2 / 3 rarity weight** that no amount of kit affinity (0.4–1.0 per tag) could overcome. Softened to **1 / 1.25 / 1.6** — a prior rather than a verdict — and re-run on the identical seed: Undying **79% → 39%**, every generic under the cap. That is a bot-competence fix first (AUGMENTS §2 promises elites respect combo lines, and they were just taking the shiniest card) and a measurement fix second. The Undying nerf was kept on its own design argument; the evidence originally cited for it did not exist.
  **Win rail:** one card over it — Bulwark Bond at 62% on **n=13**, where a winrate carries roughly ±14% standard error. Not tuned against; that is what the nightly sweep is for.
  **Side effect worth recording:** bots drafting to their kits moved the champion table too. Piper **35% → 43%** and Wisp **38% → 44%** came up into the 40–60 band (Piper also took a durability nudge), leaving **Fathom 64%** and **Grukk 61%** as the only two outside it at n≈60 — a new signal from this run, and marginal at that sample.)*
...
**Acceptance:**
- [~] 1000-match harness: zero stalemates past 20:00 (Sudden Death always resolves); event participation lifts bot winrates as designed (golem-winner advantage measurable but < 65%). — **Half met, at n=40 not n=1000** (a full match is ~13 s of wall clock here, so 1000 would be ~4 h; the run is `pnpm exec node scripts/botmatch.mjs --matches N --tier elite --vs veteran`). *Stalemates: none* — 40/40 resolved inside the 25 min cap, 30 of them via Sudden Death, average 16.9 min. *Golem advantage: **69%** over 78 takes — over the rail.* The diagnosis matters more than the number: converted golems are **killed after a mean of 18 s**, never reaching their 90 s lifetime, so the objective is not sieging and the 69% is dominated by correlation (winning the altar fight predicts winning the match). Making the golem durable enough to deliver §9's siege promise would push the number *up*, so this is a design question rather than a tuning nudge — see GAME_DESIGN §9.1. Left open deliberately.
- [x] Collapse stages swap navgrids without a single stuck unit across the harness corpus. — 60 collapse stages across 40 matches, **zero** units left standing on deleted ground; the harness fails loudly and names the units if it ever happens. (Units standing inside a live wall stamp are counted separately and are unrelated — that cell returns when the wall expires.)
...
- [x] *(added)* All five events verified in a real browser, not just in the sim. — The same script parks an offline match on each window (`?clock=`) and checks what a player would see: the announce banner with its blurb and countdown, the live chip counting *down*, the "up next" line inside its 30 s reveal, two orbs on the isles at open, the golem on the field, coins on the floor, the storm wall, and the deck narrowing 18 → 12 u. It also walks the flank route — across the light bridge to **z = −14.6**, standing on the platform with 50 s left on the chip.
...
**Ships:**
- ~~**Accounts:** guest-first identity, in-place upgrade to email+password, cross-device login, session security per TECH §10 (api + Postgres + migrations live in prod).~~ ✅ *A guest and a registered player are **the same row**: signing up is an UPDATE that attaches a credential to the account already in play, so progress survives by construction rather than by a migration step somebody has to remember. Sessions are httpOnly SameSite=Lax cookies resolved against the database on every request, so a logout or a delete takes effect immediately; mutating routes need the CSRF token too. argon2id via `hash-wasm` — pure WASM, so the VPS needs no native build tools.*
- ~~**Economy:** Clash Coins ledger, match rewards (server-validated), first-win-of-day, champion unlock flow with prices per GAME_DESIGN §18, free rotation (weekly cron), starters.~~ ✅ *Every movement writes a `transactions` row; `profiles.coins` is a cache of it, and `reconcile()` re-derives one from the other. The rotation needs **no cron**: it is a function of the week number, so a server that was switched off all week serves the right one the moment somebody logs in — same for quest dealing.*
- ~~**Hub island scene** … + full screens per UI_UX §13: Champions (3D viewer with **ability previews via FxTimeline**), Shop, **Match History** (server-side last 30, full detail view), Quests (3 daily + weekly, reroll), Mastery tracks, Profile.~~ ✅ *All six screens ship. The viewer runs the **real** FxTimelines and frames itself from each model's bounding box. **The Medieval-Hexagon island scene did not ship** — see the note below.*
- ~~Match results, quests, mastery all flow game-server→api→DB per TECH §9; offline vs-bots still works logged-out.~~ ✅ *Identity comes from an api-signed ticket, never from the client. `/internal/match-result` is the only door coins can come through, HMAC-signed over the raw body with a freshness window, and keyed on the match id so a retry is a no-op rather than a second payout. Signed out and offline are first-class states: the hub says plainly that nothing is being recorded rather than blocking.*

**Acceptance:**
- [x] Two-device flow: play on A as guest → upgrade → login on B → identical profile, history, unlocks. — `scripts/shot-hub.mjs` walks it in a real browser against a real api on one origin: guest account → buy Rattle for 3,500 → add an email → then a **completely fresh browser context** (no device key, no cookie) signs in with that email and finds the same name, the same 5,500 coins and Rattle owned. Screenshot `13-second-device.png`.
- [x] Economy integrity: coins only ever mutate through the ledger; concurrent purchase race provably idempotent; negative-balance impossible (constraint + test). — The `coins >= 0` CHECK and the partial unique index on `(user_id, idem_key)` are enforced by **real Postgres** in the tests (PGlite), not by a mock. Five concurrent purchases on one idempotency key: exactly one succeeds. Two racing purchases with money for one: exactly one succeeds. `reconcile()` finds zero drift after 300 randomised operations, and `deploy/status.sh` runs the same check against live data.
- [x] Champion detail: all 10 champions' every ability previewable in the viewer (same FxTimelines as in-game). — The viewer resolves an ability's FX keys **from the `FX` table by prefix**, so a champion cannot be missed by omission; `kit-text.test.ts` walks the whole roster asserting no description ships with an unresolved `{dmg}`. The browser smoke fires Q, W and R and asserts zero page errors — which is how two real bugs were caught (the manifest was not warmed, and timeline prop models were not preloaded).
- [x] History detail reproduces any finished match's scoreboard exactly (golden fixtures). — A real 8-seat bot match is simulated to completion, signed, posted over **real HTTP** to a real api on real Postgres, and read back: every seat's K/D/A and damage in the stored summary blob matches the stored `match_players` rows, two independent copies that would disagree if either were re-derived. The golden fixture asserts the same seed produces a byte-identical report twice, and a different seed does not.
- [x] api p95 < 120 ms for hub endpoints under 100 rps synthetic load; hub TTI budget holds with live data. — `scripts/api-load.mjs`, 40 signed-in accounts, the real endpoint mix: **100.0 rps, 0 errors, worst-endpoint p95 8.6 ms** against the 120 ms budget (`/profile` 8.6 · `/quests` 6.5 · `/champions` 4.4 · `/history` 3.5). Still clean at **200 rps** (worst p95 16.3 ms). *The run found a real defect:* the rate limiter keyed on IP alone, so 100 rps from one address — exactly what a household, a café or CGNAT looks like — was 87% 429s. It now keys per session, with the address as the fallback for anonymous traffic.

**Two things cut, on the record:**
- **The 3D hub island scene.** The screens are complete and the hub is fully playable, but the Medieval-Hexagon island with an idling favourite champion and a harbour is not built — the existing flat chrome is what ships. Cut rather than faked: a half-dressed island would have been the "programmer art" the quality bar forbids, and the screens were the part carrying the phase's promise.
- **Emote stickers and victory poses.** §18 prices all three cosmetic kinds; only **palettes** ship (2 per champion, 20 in all, previewing live in the viewer). Stickers need art and poses need animation that does not exist, and shipping coloured squares would have failed the same bar. `priceOf` returns null for them, so nothing can be bought that does not exist.
