...
- **R — Overdue Maelstrom:** ⏱ 75 s / self-centered 4.5 u, 3 s channel (move at 40% speed). A vortex of bones and burning pages: `70 (+35% AP)` arcane per 0.5 s tick; enemies leaving are slowed 30%. *Anim:* `Idle_B` upper-hold with staff overhead, procedural spin layer; GPU-instanced bone + page meshes orbit in two counter-rings, building to a slam finale.
...
- **Passive — Loose Bones:** dashing or swapping scatters 2 bone shards (4 s). Rattle's next attack consumes nearby shards: +`18 (+20% AD)` each. Shards are visible pickup-style props (readable threat).
...
- **W — War Bellow:** 25⚡ / 12 s / 4 u cone. Roar: `50 (+40% AD)` + Minis flee 1.5 s; champions dealt 20% slow and Grukk shields himself `70 (+10% max HP)`. *Anim:* `emote-no` head-shake converted to roar with jaw-open morphless head-tilt + camera micro-zoom, ring distortion VFX.
